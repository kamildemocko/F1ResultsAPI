...
func TestGetTrack(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:           "Successful request",
			url:            "/f1/api/getTrack/2023/GULF AIR BAHRAIN GRAND PRIX",
			expectedStatus: http.StatusOK,
		...
		},
		{
			...
			url:            "/f1/api/getTrack/2020/invalid",
			expectedStatus: http.StatusNotFound,
			expectedBody:   `{"code":404,"message":"error","detail":"not found"}`,
		},
		{
			name:           "Bad param request YEAR",
			url:            "/f1/api/getTrack/alpha/GULF AIR BAHRAIN GRAND PRIX",
			expectedStatus: http.StatusBadRequest,
			expectedBody:   `{"code":400,"message":"error","detail":"invalid parameter YEAR"}`,
		},
	}
...
func TestGetResults(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:           "Empty response",
			url:            "/f1/api/getResults/1933",
			expectedStatus: http.StatusNotFound,
			expectedBody:   `{"code":404,"message":"error","detail":"not found"}`,
		},
		{
			name:           "Bad param request YEAR",
			url:            "/f1/api/getResults/alpha",
			expectedStatus: http.StatusBadRequest,
			expectedBody:   `{"code":400,"message":"error","detail":"invalid parameter YEAR"}`,
		},
	}
	...
	t.Run("Successful request", func(t *testing.T) {
		...
		rr := httptest.NewRecorder()
		req, err := http.NewRequest("GET", "/f1/api/getResults/2023", nil)
		if err != nil {
			t.Fatal(err)
		}
...
func TestGetResult(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:           "Empty response",
			url:            "/f1/api/getResult/2023/1",
			expectedStatus: http.StatusNotFound,
			expectedBody:   `{"code":404,"message":"error","detail":"not found"}`,
		},
		{
			name:           "Bad param request year",
			url:            "/f1/api/getResult/alpha/30",
			expectedStatus: http.StatusBadRequest,
			expectedBody:   `{"code":400,"message":"error","detail":"invalid parameter YEAR"}`,
		},
		{
			name:           "Bad param request track id",
			url:            "/f1/api/getResult/2023/a",
			expectedStatus: http.StatusBadRequest,
	...
	t.Run("Successful request", func(t *testing.T) {
		...
		rr := httptest.NewRecorder()
		req, err := http.NewRequest("GET", "/f1/api/getResult/2023/30", nil)
		if err != nil {
			t.Fatal(err)
		}
